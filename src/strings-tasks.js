...
function sumOfCodes(str) {
  return str.split('').reduce((counter, char) => counter + char.charCodeAt(0));
}
...
function endsWith(str, substr) {
  return str.includes(substr, str.length - substr.length);
}
...
function isPalindrome(str) {
  const fixStr = str.replace(/[\W_]/g, '').toLowerCase();
  return fixStr.split('').reverse().join('') === fixStr;
}
...
function extractNameFromTemplate(value) {
  ...
  const name = value.match(template);
  return name;
}
